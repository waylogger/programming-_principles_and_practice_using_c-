/*
Antonuyk I.V. 09/08/2017

Room.cpp in Wampus_hunt
...
void Room::loop()
{
	...
	while (k->prev) 
		k = k->prev;
	if (!k) 
		throw EX_ROOM{"void Room::loop() - k is null"};
	if (!r) 
		throw EX_ROOM{"void Room::loop() - r is null"};
	r->succ = k;
	k->prev = r;
}
...
Room* Room::advice(const int arg)
{
	int step = arg;
	Room* t = this;
	if ( !t ) 
		throw EX_ROOM{"advice(int step) - !this"};
	for ( ; step > 0; --step)
	{
		if ( t -> succ ) 
			t = t -> succ; 
...
ostream& operator<< (ostream& ost, const Content& c)
{
	if 		( c == Content::none ) 		
		ost << "Empty";
	else if ( c == Content::bat ) 		
		ost << "I hear a bat!!!";
	else if ( c == Content::wampus ) 	
		ost << "I smell a wampus!!!";
	else if ( c == Content::whole ) 	
		ost << "I feel the wind!!!";
	return ost;
}
...
Room* Room::find( const Room* r )
{
	...
	//------------------------------------------------------------------------
	if ( r -> num > 20 || r -> num <= 0) 
		throw EX_ROOM {"No room with this number"};
	//------------------------------------------------------------------------
	while (true)
	{
